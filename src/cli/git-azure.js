...
program.command('app')
    ...
    .option('-d, --delete <name>', '[command] if --host specified, delete the host entry only; otherwise disable the entire application')
    ...
    .option('-cf, --certFile <file>', '[optional] file name with PKCS#7 encoded (PEM) X.509 certificate for SSL')
    .option('-k, --key <blob_name>', '[optional] the Windows Azure Blob name with PKCS#7 encoded (PEM) private key for SSL')
    .option('-kf, --keyFile <file>', '[optional] file name with PKCS#7 encoded (PEM) private key for SSL')
    .option('-a, --storageAccountName <name>', '[optional] name of the Windows Azure Blob Storage account to use')
