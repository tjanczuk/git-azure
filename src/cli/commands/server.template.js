...
<p>HTTP and WebSocket traffic is routed to your application by default if the first segment of the URL path matches the application name. \
You can disable URL path routing with:\
...
<p>HTTP and WebSocket traffic can also be routed to your application based on the host name of the URL. To associate a host name with this application, call:\
...
<p>To enable host name routing during development, you can modify the /etc/hosts file to associate your host name with the IP address of your Windows Azure service.\
...
<p>For your service to be reachable from public internet with a custom host name, you must register your host name with a public DNS registar, and set up\
...
<p>Alternatively you can also set up an A record in your DNS that points from your domain name (e.g. mydomain.com) to the IP address of your Windows Azure Service.\
 However, the CNAME method is preferred over the A method since the IP address of your service may change in certain situations.\
<h3>Enabling SSL</h3>\
<p>SSL for HTTP and WebSocket traffic is enabled by default using a self-signed X.509 certificate with the common name of CN=##AZUREHOST##.\
<p>You can specify a custom\
 SSL certificate for your application if you are using host name routing. The certificate will be served to clients that support Server Name Identification (which is virtually all modern web browsers).\
  To associate a certificate with your host name and application, call:\
...
<p>The myCertificate.pem and myKey.pem files must contain the PKCS#7 encoded (PEM) X.509 certificate and corresponding private key, respectively.\
 ...
 <p>Alternatively, you can also quickly generate a self-siged X.509 certificate corresponding to your host name with:\
 <pre>git azure app --setup ##NAME## --host myservice.com --ssl allowed --generateX509</pre>\
<p>If you need to customize your self-siged certificate, you can use the following openssl command chain to get started:\
